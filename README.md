...
| [406](ISSUE-406-storybook-vrt-coverage-gap.md) | Storybook/VRT coverage gap — harness wired in 7 workspaces; gap is story coverage (counts are an upper bound — dedup transitively; cui deprecating so start with ccc) + 2 infra gaps (circle-spaces no visual-test file, cui-icons unwired) | S3 | 🟢 partially resolved | testing (all) |
...
- **Thin VRT regression net for most of the UI** (406, under-protects 302): the `@repo/testing` snapshot harness is already wired in 7 workspaces, but stories accrue ad-hoc — ~360 story-worthy components have no story, so snapshots cover a fraction of the surface. Two infra gaps remain (`circle-spaces` has the config but no visual-test file → 0 snapshots; `cui-icons` unwired). The runtime-value half of 302 stays under-protected for the same reason. Backfilling design-system (`ccc` — **not** `cui`, which is deprecating) stories first — already wired, so each story is an instant snapshot — kills most of the class. NB the "unstoried" counts in 406 are an upper bound: dedup *transitively* (a component rendered by any story is already snapshotted), or you'll author redundant stories.
